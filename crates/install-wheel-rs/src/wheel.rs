...
pub(crate) fn write_script_entrypoints(
    ...
    for entrypoint in entrypoints {
        ...
        let launcher_python_script =
            get_script_launcher(entrypoint, &get_shebang(python_executable));
        ...
        if cfg!(windows) {
            write_file_recorded(
                site_packages,
                &entrypoint_relative,
                &windows_script_launcher(&launcher_python_script, is_gui, python_executable)?,
                record,
            )?;
        } else {
            write_file_recorded(
...
fn install_script(
    ...
    let target_path = scripts.join(file.file_name());
    ...
    let size_and_encoded_hash = if start == placeholder_python {
        let start = get_shebang(python_executable).as_bytes().to_vec();
    ...
    for entry in fs::read_dir(data_dir)? {
        ...
        match path.file_name().and_then(|name| name.to_str()) {
            Some("data") => {
                ...
                move_folder_recorded(&path, &target.data, site_packages, record)?;
            }
            Some("scripts") => {
            ...
            Some("headers") => {
                let target_path = target.include.join(dist_name);
                move_folder_recorded(&path, &target_path, site_packages, record)?;
            }
            Some("purelib") => {
                move_folder_recorded(&path, &target.purelib, site_packages, record)?;
            }
            Some("platlib") => {
                move_folder_recorded(&path, &target.platlib, site_packages, record)?;
            }
            _ => {
                return Err(Error::InvalidWheel(format!(
